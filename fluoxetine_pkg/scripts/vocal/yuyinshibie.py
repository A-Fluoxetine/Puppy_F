...
import rospy  # ROS Noetic Python客户端库
from std_msgs.msg import String  # ROS消息类型
...
def speech_to_text(filename, engine, publisher=None):
    try:
        ...
        for keyword, action in KEYWORD_ACTIONS.items():
            ...
            if similarity >= 80:
                ...
                # 发布关键词到ROS话题
                if publisher:
                    publisher.publish(keyword)
                    rospy.loginfo(f"发布关键词: {keyword}")
...
def main():
    ...
    publisher = rospy.Publisher('keyW', String, queue_size=10)
    rospy.loginfo("已创建keyW话题发布者")
...
#此处想在电脑上用的话记得更改一下默认录音设备。一般改为0就行，这里的13是指树莓派上的录音代号S
    ...
    try:
        speak(engine, "请开始说话。")
        while not rospy.is_shutdown():  # 修改循环条件以兼容ROS
            if recorder.process_audio():
                ...
                # 语音识别
                text = speech_to_text(filename, engine, publisher)
                if text:
                    print(f"\n识别结果：{text}")
                    # 单句录音结束后提示“汪”
                    speak(engine, "汪")
